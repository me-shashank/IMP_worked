...
run() {
    ...
    MINIFI_CONFIG_YML="${MINIFI_HOME}/conf/config.yml"
    AUDIT_SECRET=$( cat /mnt/auditdb-secret.txt )
    SOURCE_SECRET=$( cat /mnt/source-secret.txt )
    #Set the audit db secret from mount path of VM Instance
    sed -i -e 's/SECRETFROMKMSAUDIT/'"$AUDIT_SECRET"'/g' ${MINIFI_CONFIG_YML}
    sed -i -e 's/SECRETFROMKMSSOURCE/'"$SOURCE_SECRET"'/g' ${MINIFI_CONFIG_YML}
